...
- 优先保持现有架构风格
- 新功能和后续重构优先按 MVVM 设计模式组织代码，尽量分离 View、ViewModel、Model/Repository 职责
- 优先局部修改，不要大范围重构
- 除非明确要求，否则不要修改公共接口
- 不要引入新的第三方库
- 不要修改无关文件
...
- 非必要不定义静态、全局函数
- 非必要不定义静态、全局变量
- 函数命名优先直接表达功能语义，避免含糊缩写
- 可复用的公共工具逻辑优先封装到明确职责的类中，不要散落为全局方法
- 关键变量、数据结构字段、数据库字段需要补充简洁注释，便于后续维护理解
